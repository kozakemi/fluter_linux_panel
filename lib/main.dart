...
// 全局配置变量
const bool SHOW_SECONDS = true; // 控制是否显示秒

// 使用相对比例而不是固定尺寸
const double SIDE_PANEL_WIDTH_RATIO = 0.2; // 侧边栏宽度占屏幕宽度的比例
...
class _ClockScreenState extends State<ClockScreen>
  ...
  Widget build(BuildContext context) {
    ...
    // 计算侧边栏宽度
    final sidePanelWidth = screenWidth * SIDE_PANEL_WIDTH_RATIO;
      ...
      children: [
        ...
        ValueListenableBuilder<bool>(
          ...
          builder: (context, wifiConnected, child) {
            return Positioned(
              left: (screenWidth - sidePanelWidth) / 3 -(0.08*screenHeight)/2, 
              // y = x*k+b-iconSize/2
              top: ((screenWidth - sidePanelWidth) / 3 -(0.08*screenHeight)/2 )*(-0.15 * screenHeight)/(screenWidth - sidePanelWidth) + (0.2*screenHeight)-(0.08*screenHeight)/2,
        ...
        ValueListenableBuilder<bool>(
          ...
          builder: (context, mqttConnected, child) {
            return Positioned(
              left: (screenWidth - sidePanelWidth) / 3 * 2 -(0.08*screenHeight)/2,
              top: ((screenWidth - sidePanelWidth) / 3 *2 -(0.08*screenHeight)/2 )*(-0.15 * screenHeight)/(screenWidth - sidePanelWidth) + (0.2*screenHeight)-(0.08*screenHeight)/2,
              child: _buildStatusIcon(
                context,
                mqttConnected
  ...
  Widget build(BuildContext context) {
    ...
    return ValueListenableBuilder<DateTime>(
      ...
      builder: (context, currentTime, child) {
        // 根据全局变量控制是否显示秒
        final timeFormat = DateFormat(SHOW_SECONDS ? 'HH:mm:ss' : 'HH:mm');
